...
\begin{model}[h]
  \centering
  \titlecaption{Model Equations for the Expanded Model}{These equations have similar reaction stoichiometry to those in table \ref{mod:hirota}, but with more parametric degrees of freedom. This model showed better time-series performance than the more constrained model when fit to time-series data.}
...
\begin{table}[p]
  \titlecaption{Parameter Set for Expanded Model}{Parameters for model described in \fref{mod:hirota2}, fit to time-series data via nonlinear programming.}
  \label{tab:parsetopt}
  \vspace{2mm}
  \centering
