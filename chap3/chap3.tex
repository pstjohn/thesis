% \section{Motivation for confidence in model predictions (link to previous chapter)}\blindtext
% \section{Efficient parameter optimization using collocation methods}\blindtext
% \subsection{Implicit ODE solution}\blindtext
% \subsection{Nonlinear programming}\blindtext
% \section{Method results}\blindtext
% \subsection{Effect of data quality}\blindtext
% \subsection{Effect of sampling frequency}\blindtext
% \subsection{Effect of model structure}\blindtext
...
The $\tau_k$ values are chosen for optimal accuracy, here we use Gauss-Radau
...
Time-dependent parametric sensitivities, $$\mathbf{S}(t) \equiv \frac{\partial
...
For the {\itshape in silico} data of varying quality used in Figures 2-4, we
...
After finding optimal parameter fits, we used the models to predict how
...
\begin{figure}[h]
  ...
  \titlecaption{Time-course Profiles of the State Trajectories for Per mRNA}{(A) Increasing relative error, $\xi$, with ${\cal M} = 20$. Possible state variable values are shown as shaded regions, obtained by filling between the 5$^\text{th}$ and 95$^\text{th}$ percentile for values at each time for 2000 independent parameter estimations. Increasing $\xi$ results in larger deviations from the original model trajectory, shown as a dashed black line.\\ (B) Decreasing number of measurement points, $\cal M$, each with $\xi = 0.15$.  Higher $\cal M$ results in trajectories closer to the true trajectory.}
  \label{fig:3_2}
\end{figure}

Figure 3 shows violin plots of the probability distribution for each parameter
...
\begin{figure}[h]
  ...
  \titlecaption{Effect of High-resolution Sampling on Identifiability}{ Lower values of $\cal M$ result in less constrained parameter and sensitivity values. Similar to Figure 3, violin plots of the parameters (left) and sensitivities (right) show the distribution from each parameter estimation for decreasing $\cal M$. These results highlight the importance of high-resolution time sampling in generating sensitivity information for oscillatory models.}
  \label{fig:3_4}
\end{figure}
...
\begin{table}[h]
  \centering
    \titlecaption{Model Equations for the Expanded Model}{These
      equations have similar reaction stoichiometry to those in table
      \ref{tab:modelfns}, but with more parametric degrees of
      freedom. This model showed better time-series performance than
      the more constrained model when fit to time-series data. \label{tab:modelfns2}}
...
\begin{table}[h]
  \titlecaption{Parameter Set for Expanded Model}{Parameters for model described
    in table~\ref{tab:modelfns2}, fit to time-series data via nonlinear
  programming.}
  \label{tab:parsetopt}
  \vspace{2mm}
  \centering
...
\section{Conclusions}
Increasingly, mathematical models are being used to study biological systems
where traditional experiments would prove infeasible.  For example, in the
search for drug targets, thousands of possible combinatorial perturbations can
be quickly scanned for therapeutic effects using {\itshape in silico} modeling.
This is especially useful in oscillatory systems with long periods, such as
circadian rhythms, where a perturbed {\itshape in vitro} or {\itshape in vivo}
system must be measured for multiple days before changes can be reliably
determined.
 
However, since errors in model responses can arise from either incorrect
