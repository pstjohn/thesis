...
Such rhythms are known as {\itshape circadian}, and are classified by a number of defining characteristics:
...
An equivalent sensitivity analysis for a limit-cycle model raises additional complications. 
First, finding the derivative of the period with respect to parameter values requires a clever decomposition of the sensitivity matrices \cite{Kramer1984}. 
Period sensitivities, $\frac{\partial T}{\partial {\bm p}}$, can be obtained directly from sensitivities integrated for one pass of the limit cycle (derived in \cite{Wilkins2009}) through a linear solve:
\begin{equation}
  \left[\begin{array}{cc}
...
Additionally, perturbations to state variables on the limit cycle results in a change of phase, which can be quantified into phase sensitivities. 
