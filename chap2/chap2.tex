...
This chapter describes a collaborative project in which experimental results inspired the creation of a new model of the core mammalian feedback network.
...
In order to explain these new experimental results while maintaining consistency with published evidence, a simple mathematical model of the PER/CRY negative feedback loop was constructed.
...
Using new data from the application of KL001 together with existing experimental results, a mechanism was hypothesized to reconcile these seemingly contradictory results.
The experimentally motivated assumptions underlying this hypothesized mechanism are described in the following sections, with descriptions of how they are supported by the literature.

\subsubsection{Assumptions on network structure}
...
PER and CRY bind and enter the nucleus in a 1:1 stoichiometric ratio, similar to more complicated models of mammalian circadian rhythms \cite{Leloup2003,Mirsky2009,Forger2003}.

\subsubsection{Assumptions on protein stoichiometry}
...
\subsubsection{Assumptions on protein degradation}
...
\subsubsection{Assumptions on time delays}
...
In order to capture these dynamics without introducing unnecessary stiffness, only the concentrations of nuclear mRNA, cytosolic unbound protein, and nuclear protein complexes are explicitly considered.
...
\begin{figure}[bt]
  ...
  \titlecaption{Connectivity of \fref{mod:hirota}}{
    ...
    The wild-type period, consisting of both loops, is a balance between the short and long oscillations. Figure reprinted from \cite{Hirota2012}.} \label{fig:twoloops}
\end{figure}
...
First, CRY1 and CRY2 were allowed to have different inhibitory efficiency, as demonstrated experimentally \cite{GriffinJr.1999}. 
...
To this end, the degradation rates of nuclear CRY1 and CRY2 were allowed to differ, while their inhibition constants were set to be equal. 
...
\begin{figure}[bt]
  ...
  \titlecaption{Repressor degradation profiles}{Time course plots of the simulated total nuclear repressor concentration for the degradation model under various clock conditions. The relative contribution of CRY1 and CRY2 are shaded green and orange, respectively. Longer periods occur when a higher percentage of the nuclear complex is CRY1, taking longer to degrade. Figure reprinted from \cite{Yang2014}.}
...
\begin{figure}[bp]
  \centering
  \includegraphics[width=0.5\textwidth]{chap2/figures/cryseperatedegradation.pdf}
...
The rate equations associated with a shared-enzyme degradation mechanism are derived from the equilibrium relationships:
\begin{equation*}
...
\end{align}
If the same operations are performed for [ES2] and plugged into \fref{eq:eqabove}.
\begin{align}
...
\end{align}
The second fraction from \fref{eq:twofracs} is then pulled into the denominator of the first and simplified further.
\begin{align*}
...
A genetic algorithm strategy was employed to minimize the cost function, which was used previously for optimizing circadian parameters \cite{Mirsky2009}. 
In the method, 5000 solutions are calculated at random parameter values chosen on a log scale, with $p_{init} \in (10^{-3}, 10^1)$.
...
\begin{longtable}{cp{6.0cm}crrr}
  \titlecaption{Summary of cost function entries}{
    ...
    Entries 1-9 and 14-18 were obtained from the data presented in \cite{Lee2001}. 
    SiRNA sensitivities were obtained were taken from \cite{Zhang2009}.}\\ \toprule \label{tab:cost} 
    & Description & Weight & Desired [ref] & Degradation & Activation \\
    \midrule
  \endfirsthead
...
\bottomrule
\endfoot
1  & {\it Per} mRNA peak-trough ratio $$\frac{y_\text{max}(\text{\it Per})}{y_\text{min}(\text{\it Per})}$$                                                & 0.5 & $> 20$  \cite{Lee2001}           & large       & large      \\
2  & {\it Cry1} mRNA peak-trough ratio $$\frac{y_\text{max}(\text{\it Cry1})}{y_\text{min}(\text{\it Cry1})}$$                                             & 0.5 & $2.155$ \cite{Lee2001}           & 8.942       & 3.748      \\
3  & {\it Cry2} mRNA peak-trough ratio $$\frac{y_\text{max}(\text{\it Cry2})}{y_\text{min}(\text{\it Cry2})}$$                                             & 0.5 & $2.236$ \cite{Lee2001}           & 7.813       & 3.484      \\
4  & PER protein peak to trough ratio $$\frac{y_\text{max}(\text{PER})}{y_\text{min}(\text{PER})}$$                                                        & 5   & $> 20$  \cite{Lee2001}           & large       & large      \\
5  & CRY1 protein peak-trough ratio $$\frac{y_\text{max}(\text{CRY1})}{y_\text{min}(\text{CRY1})}$$                                                        & 3   & $3.247$ \cite{Lee2001}           & 6.385       & 1.847      \\
6  & CRY2 protein peak-trough ratio $$\frac{y_\text{max}(\text{CRY2})}{y_\text{min}(\text{CRY2})}$$                                                        & 3   & $1.975$ \cite{Lee2001}           & 8.094       & 2.347      \\
7  & Fraction PER of total protein $$\frac{y_\text{max}(\text{PER})}{y_\text{max}(\text{PER}) + y_\text{max}(\text{CRY1}) + y_\text{max}(\text{CRY2})}$$   & 3   & $0.105$ \cite{Lee2001}           & 0.169       & 0.073      \\
8  & Fraction CRY1 of total protein $$\frac{y_\text{max}(\text{CRY1})}{y_\text{max}(\text{PER}) + y_\text{max}(\text{CRY1}) + y_\text{max}(\text{CRY2})}$$ & 3   & $0.555$ \cite{Lee2001}           & 0.473       & 0.554      \\
9  & Fraction CRY2 of total protein $$\frac{y_\text{max}(\text{CRY2})}{y_\text{max}(\text{PER}) + y_\text{max}(\text{CRY1}) + y_\text{max}(\text{CRY2})}$$ & 3   & $0.341$ \cite{Lee2001}           & 0.358       & 0.373      \\
10 & {\it Cry1} siRNA period sensitivity $$\frac{\partial T}{\partial v_{\text{deg,{\bf c1}}}}$$                                                           & 5   & $< 0$   \cite{Zhang2009}         & $< 0$       & $> 0$      \\
11 & {\it Cry2} siRNA period sensitivity $$\frac{\partial T}{\partial v_{\text{deg,{\bf c2}}}}$$                                                           & 5   & $> 0$   \cite{Zhang2009}         & $> 0$       & $< 0$      \\
12 & {\it Cry1} knockout period $$\frac{T(\text{\it Cry1}^{-/-})}{T(\text{WT})}$$                                                                          & 5   & $< 95\%$  \cite{VanderHorst1999} & $92.1 \%$   & $193.4 \%$ \\
13 & {\it Cry2} knockout period $$\frac{T(\text{\it Cry2}^{-/-})}{T(\text{WT})}$$                                                                          & 5   & $> 115\%$ \cite{VanderHorst1999} & $131.8 \% $ & $99.4 \%$  \\
14 & Fraction CRY1 entering nucleus $$\frac{y_\text{max}(\text{CRY1}_n)}{y_\text{max}(\text{CRY1} + \text{CRY1}_n)}$$                                      & 1   & $0.40$    \cite{Lee2001}         & $0.195$     & 0.059      \\
15 & Fraction CRY2 entering nucleus $$\frac{y_\text{max}(\text{CRY2}_n)}{y_\text{max}(\text{CRY2} + \text{CRY2}_n)}$$                                      & 1   & $0.35$    \cite{Lee2001}         & $0.139$     & 0.059      \\
16 & Time delay between nuclear repressors and mRNA $$t_\text{max}(\text{nuclear protein}) - t_\text{max}(\text{mRNA})$$                                   & 3   & $75 \%$   \cite{Lee2001}         & $0.850$     & 0.848      \\
17 & Time delay between mRNA and cytoplasmic protein $$t_\text{max}(\text{mRNA}) - t_\text{max}(\text{protein})$$                                          & 3   & $25 \%$ \cite{Lee2001}           & $0.057$     & 0.095      \\
18 & Time delay between cytoplasmic protein and nuclear protein $$t_\text{max}(\text{protein}) - t_\text{max}(\text{nuclear protein})$$                    & 3   & $0 \%$  \cite{Lee2001}           & $0.093$     & 0.057      \\
\end{longtable}

\begin{table}[p]
  \titlecaption{Best-fit parameter values}{Best-fit parameters for the two alternative models proposed for the core clock feedback loop, as determined by fitting the cost function entries in \fref{tab:cost}.}
  \label{tab:parset}
  \vspace{2mm}
  \centering
  \begin{tabular}{cllrr} \toprule
    & Parameter                 & Description                       & \fref{mod:hirota} & \fref{mod:actfns} \\ \midrule
...
The model was validated by comparing the simulated dynamics to novel experimental measurements. 
...
\begin{figure}[bt]
  ...
  \titlecaption{Time course concentration profiles of \fref{mod:hirota}}{
    ...
    High levels of nuclear repressors halt transcription until both CRYs have degraded (right plot). Figure reprinted from \cite{Hirota2012}.}
...
\begin{figure}[bt]
  ...
  \titlecaption{Validation of \fref{mod:hirota} against experimental results}{
...
Possible mechanisms by which KL001 might lengthen the circadian period were investigated using the completed model. 
...
This prediction was subsequently verified via structural biology, in which KL001 was shown to interfere directly with the binding of FBXL3 and CRY1 \cite{Nangle2013}. 
\begin{figure}[tbp]
  \centering
  \includegraphics{chap2/figures/fbxl3kl001data.pdf}
  \titlecaption{FBXL3 knockdown interferes with period-lengthening effect of KL001}{The dose-dependent period increase of KL001 (right, black line) is abolished in cells treated with FBXL3 siRNA, indicating the mechanism of KL001 acts through FBXL3 and confirming model predictions. Figure reprinted from \cite{Hirota2012}.}
  \label{fig:fbxl3data}
\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.7\textwidth]{chap2/figures/crysimultaneousknockdown.pdf}
  \titlecaption{Simultaneous knockdown of {\it Cry1} and {\it Cry2}}{Period contours (\% change) resulting from simultaneous {\it in silico} siRNA knockdowns of {\it Cry1} (x-axis) and {\it Cry2} (y-axis). Since moving along the $y=x$ line remains close to the $100\%$ period contour, the CRY1/CRY2 ratio largely determines the period, independent of total CRY, for perturbations up to twice the normal mRNA degradation rate.}
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.7\textwidth]{chap2/figures/firstordersens.pdf}
