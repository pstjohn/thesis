...
\begin{figure}[bt]
  ...
  \begin{minipage}{0.8\textwidth}
    ...
    \titlecaption{Model Connectivity}{Schematic for the core negative feedback loop, consisting of the two redundant {\it Cry} mechanisms. The wild-type period, consisting of both loops, is a balance between the short and long oscillations.} \label{fig:twoloops}
  \end{minipage}
\end{figure}
...
\begin{figure}[bt]
  ...
  \begin{minipage}{0.9\textwidth}
    ...
    \titlecaption{Degradation Profiles}{Time course plots of the simulated total nuclear repressor concentration for the degradation model under various clock conditions. The relative contribution of CRY1 and CRY2 are shaded green and orange, respectively. Longer periods occur when a higher percentage of the nuclear complex is CRY1, taking longer to degrade.}
...
\section{Parameter Estimation}\label{sec:parameter}
...
& 3 & $0 \%$ & $0.093$ & 0.057 \\
  ...
  \begin{figure}[bt]
    ...
    \begin{minipage}{0.9\textwidth}
      ...
      \titlecaption{Time Course Plots}{The three polar plots show the time-varying levels of each clock component for optimal parameter set. In these plots, the amplitude of each variable (always positive) is plotted against its phase, $0 \rightarrow 2\pi$. Since the limit cycle is periodic, this results in a closed curve. Rising mRNA levels caused by low repressor concentrations (CT12) result in accumulating cytosolic protein (left plot). Lower levels of PER prevent all the available CRY from entering the nucleus (middle plot). High levels of nuclear repressors halt transcription until both CRYs have degraded (right plot).}
  ...
  \begin{figure}[bt]
    ...
    \begin{minipage}{\textwidth}
      ...
      \titlecaption{Model Validation}{When comparing against existing experimental results, the model shows the correct response for {\it Cry1} siRNA, {\it Cry2} siRNA, and $\text{CRY}_c$ stabilization (left, middle), through an adjustment in the nuclear CRY1/CRY2 ratio. The model also correctly captures the single/double knockout phenotype of \cite{VanderHorst1999}, (right)} \label{fig:experimentalvalidation}
    \end{minipage}
  \end{figure}
  ...
  \section{Insights into Circadian Network Design}
  ...
  \begin{figure}[bt]
    ...
    \begin{minipage}{0.8\textwidth}
      ...
      \titlecaption{Simultaneous Knockdown of {\it Cry1} and {\it Cry2}}{Period contours (\% change) resulting from simultaneous {\it in silico} siRNA knockdowns of {\it Cry1} (x-axis) and {\it Cry2} (y-axis). The plot shows that the CRY1/CRY2 ratio determines the period, independent of total CRY, for perturbations up to twice the normal mRNA degradation rate.}  \label{fig:simultaneouskdown}
    \end{minipage}
  \end{figure}
  ...
  \begin{figure}[bt]
    ...
    \begin{minipage}{0.8\textwidth}
      \centering
      \includegraphics[width=0.8\textwidth]{chap2/figures/firstordersens.pdf}
