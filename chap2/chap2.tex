...
\begin{table}[p]
  \centering
  \titlecaption{Changed Equations for the Activation-based Model}{The remainder of the model equations (not duplicated below) are found in  \fref{tab:modelfns}}\label{tab:actfns}
...
\begin{table}[p]
  \titlecaption{Parameter Set}{Parameters for model described in \fref{tab:modelfns}.}
  \label{tab:parset}
  \vspace{2mm}
  \centering
...
To derive the equations for the Hill-type inhibition, allowing for different repressive activities (\fref{tab:actfns}), we start with the standard equation for Hill-type regulation:
...
A description of each entry in the cost function, along with the value for experimental and final model is shown in \fref{tab:cost} (degradation-based model, \fref{tab:modelfns}; activation-based model, \fref{tab:actfns}). 
